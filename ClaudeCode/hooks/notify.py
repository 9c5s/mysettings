...
def main() -> None:
    ...
    try:
        notifier = DesktopNotifierSync(app_name=APP_NAME, app_icon=_get_app_icon())
    except RuntimeError, OSError:
        return

    notifier.send(
        title=_render(template.title, values),
        message=_render(template.message, values),
